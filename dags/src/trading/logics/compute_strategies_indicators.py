...
class computeStrategiesIndicators:
    ...
    def strategy_sp500_15m_v2(self, inputs_dict):
        ...
        # Compute Market gain
        data_e2_df['market_gain'] = data_e2_df['Close'] - data_e2_df['Open_adj'] 

        market_gain_df = data_e2_df[data_e2_df['market_gain'].isnull()==False][['market_gain']]
        market_gain_df['mg_bucket'] = pd.qcut(market_gain_df['market_gain'],11, labels=False)

        obs_df = market_gain_df.groupby(
            ['mg_bucket'],
            as_index=False
        ).agg(
            min_s= ('market_gain','min'),
            max_s = ('market_gain','max'),
            avg_s = ('market_gain','mean'),
            frequence = ('market_gain','count'),
        ).copy()
